...
public class additional_ass2 {
	...
	public static String scanNum1() {// 조건 h, m, s로 구분해야한다. 한칸띄어쓰기를 지켜준다.
		...
		String firstSpace = scanner.nextLine();
		String first = firstSpace.replace(" ", "");
		return first;
	}

	public static String scanNum2() {// 조건 h, m, s로 구분해야한다. 한칸띄어쓰기를 지켜준다.
		System.out.println("input the first time. ex)00h 00m 00s");
		Scanner scanner = new Scanner(System.in);
		String secondSpace = scanner.nextLine();
		String second = secondSpace.replace(" ", "");
		return second;
	}

	public static void calculate(String first, String second) {// first second 받은 담으로 계산한다.
		...
		String strmm1 = first.substring(hour1 + 1, mm1);
		int mm1Int = Integer.parseInt(strmm1);
		int ss1 = first.indexOf("s");
		String strss1 = first.substring(mm1 + 1, ss1);
		...
		String strmm2 = second.substring(hour2 + 1, mm2);
		int mm2Int = Integer.parseInt(strmm2);
		int ss2 = second.indexOf("s");
		String strss2 = second.substring(mm2 + 1, ss2);
